...
class Customers extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, l_name, email, company, address, city, state, zip, phone, signed_up, customer, subscribed', 'required'),
			array('zip, checked_in', 'numerical', 'integerOnly'=>true),
			array('name, l_name, email, company, city, phone, customer', 'length', 'max'=>100),
			array('address', 'length', 'max'=>500),
			array('state', 'length', 'max'=>2),
			array('subscribed', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('ID, name, l_name, email, company, address, city, state, zip, phone, signed_up, checked_in, customer, subscribed', 'safe', 'on'=>'search'),
		);
	}
